...
Specifies extra arguments to be passed to C<pppd> to the default
set of I<nodetach notty local logfd 2 nopersist>.

Defaults to I<dump debug 172.31.82.1:172.31.82.2>.
...
my $name;
my @pppd = qw/dump debug 172.31.82.1:172.31.82.2/;
while (@ARGV) {
	$_ = shift @ARGV;
	if ($_ eq '--') {
		@pppd = @ARGV;
...
sub send_netlink
{
	...
	send $fd, "$head$props", 0, pack 'SSLL', AF_NETLINK, 0, 0, 0x0002 or
		die "Can't send a netlink message: $!";
}
...
while (<$pty>) {
	...
	if (/^AT$/ or /^ATE0$/ or /^ATV1$/ or /^AT\+CMEE=1$/ or /^ATX4$/ or /^AT&C1$/ or /^ATZ$/) {
		...
		if ($ppp == 0) {
			...
			close $pty;
			exec qw/unshare --net pppd nodetach notty local logfd 2 nopersist/, @pppd;
			die "Can't exec pppd: $!";
		}
		waitpid $ppp, 0;
	} else {
