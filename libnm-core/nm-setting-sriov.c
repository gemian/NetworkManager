...
struct _NMSettingSriov {
	...
	guint total_vfs;
	GPtrArray *vfs;
	NMBooleanValue autoprobe_drivers;
};
...
nm_sriov_vf_get_attribute_names (NMSriovVF *vf)
{
	...
	names = nm_utils_strdict_get_keys (vf->attributes, TRUE, NULL);
...
static int
compare_vlan_id (gconstpointer a, gconstpointer b, gpointer user_data)
{
	guint id_a  = *(guint *) a;
	guint id_b  = *(guint *) b;
...
nm_sriov_vf_get_vlan_ids (NMSriovVF *vf, guint *length)
{
	...
	g_qsort_with_data (vf->vlan_ids, num, sizeof (guint), compare_vlan_id, NULL);
...
{
	g_return_val_if_fail (NM_IS_SETTING_SRIOV (setting), NM_BOOLEAN_VALUE_DEFAULT);
...
nm_setting_sriov_class_init (NMSettingSriovClass *setting_class)
{
	 ...
	 * If set to %NM_BOOLEAN_VALUE_DEFAULT, the global default is used and
	...
	g_object_class_install_property
		(object_class, PROP_AUTOPROBE_DRIVERS,
		 g_param_spec_enum (NM_SETTING_SRIOV_AUTOPROBE_DRIVERS, "", "",
		                    nm_boolean_value_get_type (),
		                    NM_BOOLEAN_VALUE_DEFAULT,
		                    NM_SETTING_PARAM_FUZZY_IGNORE |
		                    G_PARAM_READWRITE |
		                    G_PARAM_CONSTRUCT |
		                    G_PARAM_STATIC_STRINGS));
